...
# Object detection imports from the module
        ...
        with tf.compat.v1.Session(graph=detection_graph,
            ...
            while input.isOpened():
                ...
                for i, b in enumerate(boxes[0]):
                    if classes[0][i] == 1 or classes[0][i] == 3 or classes[0][i] == 6 or classes[0][i] == 8:  # if it's a car, a truck or a bus
                        if scores[0][i] > 0.5:  # if the confidence level is bigger than 50%
                            mid_x = (boxes[0][i][1] + boxes[0][i][3]) / 2
                            mid_y = (boxes[0][i][0] + boxes[0][i][2]) / 2
                            aproxDist = round((1 - (boxes[0][i][3] - boxes[0][i][1])) ** 4,
                                              1)  # the power can be tweaked,
...
def main():
    ...
    psg.theme("Dark")
    buttonsLine = [
        [psg.Button("START DETECTION", size=(20, 10)),
         psg.Button("TEST DETECTION", size=(20, 10), button_color='Blue'),
         psg.Button("SETTINGS", size=(20, 10), button_color='Gray'),
         psg.Button("EXIT", size=(20, 10), button_color='Red')]
    ]
    # Layout window config
    layout = [
        [psg.Text("Traffic Participants Detection", size=(120, 40), justification="center")],
        [psg.Image(filename="", key="-IMAGE-")],
        [psg.Column(buttonsLine, vertical_alignment='center', justification='center', k='-C-')]
    ]

    window = psg.Window("TPD", layout, location=(800, 400), resizable=True, finalize=True)
    while True:
        event, values = window.read(timeout=20)
        if event == "START DETECTION":
            detection(videoInput)
